  ...
  return (
    <div className="mb-4">
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-[15px] font-mono text-[#C8CCC8] truncate max-w-[120px]">{label}</span>
        <span className="text-[15px] font-mono tabular-nums font-semibold" style={{ color }}>
          {edgePct > 0 ? '+' : ''}{edgePct.toFixed(1)}%
        </span>
      </div>
      {/* Model bar */}
      <div className="flex items-center gap-2.5 mb-1.5">
        <span className="text-[12px] font-mono text-[#6B6F6B] w-8 text-right flex-shrink-0">MOD</span>
        ...
        <span className="text-[14px] font-mono tabular-nums text-[#C8CCC8] w-9 flex-shrink-0">{modelPct}%</span>
      </div>
      {/* Market bar */}
      <div className="flex items-center gap-2.5">
        <span className="text-[12px] font-mono text-[#6B6F6B] w-8 text-right flex-shrink-0">MKT</span>
        <div className="flex-1 h-2 bg-[#1E201E] rounded-full overflow-hidden">
          <div
        ...
        <span className="text-[14px] font-mono tabular-nums text-[#6B6F6B] w-9 flex-shrink-0">{marketPct}%</span>
      </div>
    </div>
  );
}
...
export function ModelVsMarketChart({ markets }: { markets: MarketAnalysis[] }) {
  ...
  return (
    <div className="space-y-6">
      <div className="text-[13px] font-mono text-[#6B6F6B] uppercase tracking-widest">
        Modelo vs Mercado
      </div>
      {markets.map((market) => (
        <div key={market.market}>
          <div className="text-[12px] font-mono text-[#9A9E9A] uppercase tracking-wider mb-3">
